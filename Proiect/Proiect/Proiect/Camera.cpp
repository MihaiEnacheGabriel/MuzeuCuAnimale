...
void Camera::updateMatrix(float FOVdeg, float nearPlane, float farPlane)
{
	...
	cameraMatrix = GetViewMatrix() * GetProjectionMatrix();
}
...
void Camera::Inputs(GLFWwindow* window)
{
	...
	{
		ProcessKeyboard(FORWARD, 1.0f);
	}
	if (glfwGetKey(window, GLFW_KEY_A) == GLFW_PRESS)
	{
		ProcessKeyboard(LEFT, 1.0f);
	}
	if (glfwGetKey(window, GLFW_KEY_S) == GLFW_PRESS)
	{
		ProcessKeyboard(BACKWARD, 1.0f);
	}
	if (glfwGetKey(window, GLFW_KEY_D) == GLFW_PRESS)
	{
		ProcessKeyboard(RIGHT, 1.0f);
	}
	if (glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS)
	{
		ProcessKeyboard(UP, 1.0f);
	}
	if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS)
	{
		ProcessKeyboard(DOWN, 1.0f);
	}
